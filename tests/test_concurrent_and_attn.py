...
def test_e1_openrouter_config_has_cost_and_retry_settings() -> None:
    ...
    assert all(m.max_tokens == 2048 for m in cfg.methods)
    assert {m.name for m in cfg.models} == {"qwen3.5-9b", "qwen3.5-27b", "gemma3-4b", "llama3.2-3b", "deepseek-v4-flash", "deepseek-v4.1-flash"}
...
def test_e1_ramp_router_config_uses_responses_backend() -> None:
    ...
    assert cfg.max_concurrent == 4
    assert all(m.max_tokens == 2048 for m in cfg.methods)
    assert {m.name for m in cfg.models} == {"qwen3.5-4b", "qwen3.5-9b", "qwen3.5-27b", "gemma3-4b", "llama3.2-3b", "deepseek-v4-flash", "deepseek-v4.1-flash"}
    for model in cfg.models:
